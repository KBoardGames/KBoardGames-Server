...
import vendor.mtwin.mail.Part;
import vendor.mtwin.mail.Smtp;
import neko.Lib;
...
class EasyMail {
	...
	public function send(from:String, to:String, smtpHost:String, port:Int=25, username:String="", password:String=""):Void {
		var mail = new vendor.mtwin.mail.Part("multipart/alternative");
		...
		try {
			vendor.mtwin.mail.Smtp.send(smtpHost, from, to, mail.get(), port, username, password);
		}
		catch (e:Exception) {
			switch(e) {
